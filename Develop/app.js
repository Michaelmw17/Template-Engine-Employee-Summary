...
//Function begins
start = async () => {
    console.log("Let's Enter your Team!");
    ...
    await inquirer.prompt(
        {
            type: "number",
            message: "How many people are in the team?",
            name: "noOfTeamMem"
        }
    )
    .then((data) => {

        // Number of team member's placed in teamSize for scope purposes.
        // 1 is added start from 1  not 0 for user understanding.
        teamSize = data.noOfTeamMem + 1;
    });
    
    // If Team Size is 0, will end
    if (teamSize === 0){
        return;
    }
    ...
    for(i = 1; i < teamSize; i++){
        ...
        await inquirer.prompt([ 
            ...
            {
                type: "input",
                message: `What's the employee (${i})'s id?`,
                name: "id"
            },
            {
                type: "input",
                message: `What's the employee (${i})'s Email?`,
                name: "email"
            },
            {
                type: "list",
                message: `What's the employee (${i})'s title?`,
                name: "title",
